...
class ChequeBookRequestPage extends StatefulWidget {
  ...
  static void navigateTo(BuildContext context, Account account) {
    Navigator.of(context).push(MaterialPageRoute(
        builder: (_) => ChequeBookRequestPage(account)
    ));
  }
  ...
  _submitRequest() async {
    ...
    if (response.code == 200) {
      await MessageDialog.show(
          context,
          'Your check book request was successfully sent',
          'Request Sent'
      );
      return Navigator.of(context).pop();
    }
    MessageDialog.show(context, response.message, response.description);
    setState(() {
      _loading = false;
    });
  }
                        ...
                        Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                // ),
                                // SizedBox(height: 8),
                                Text('Select the account below for which you want to request a cheque book')
                              ],
                            )
                        ),
                        Divider(height: 1),
